            ...
            .map(|d| {
                ...
                Ok(proto::ListDevicesResponse {
                    device: Some(proto::DeviceInfo {
                        ...
                        platform: 0,
                        last_seen: 0,
                        // Empty until the blob is stored and returned — see S1 in
                        // construct-docs/backend/SERVER_TASKS_2026-09-03.md. The client reads a
                        // device's name and platform out of this and falls back to the short
                        // device id while it is empty, which is what every row shows today.
