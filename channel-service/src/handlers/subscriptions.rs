...
pub(crate) async fn subscribe_channel(
    ...
    // For PRIVATE channels, check invite token
    ...
    // Check max_subscribers limit (0 = unlimited)
    ...
    // Fetch existing Sender Key if available
...
pub(crate) async fn unsubscribe_channel(
    ...
    if !removed {
        // Could be owner trying to unsubscribe
    ...
    // Optionally rotate Sender Key for this device
    ...
    let channels: Vec<proto::ChannelInfo> = channels_list
        ...
        .map(|c| {
            ...
            proto::ChannelInfo {
                ...
                subscribed_at: 0, // populated from channel_subscribers join
            }
        })
        .collect();
